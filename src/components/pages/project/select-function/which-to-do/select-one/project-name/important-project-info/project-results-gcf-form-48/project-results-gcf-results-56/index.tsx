...
const [responseData] = useState(generateResult || state?.response);
  ...
  useEffect(() => {
    if(localStorage.getItem('allResults') === null){
      localStorage.setItem('allResults', JSON.stringify([{result7: state?.response || ""}]))
    }
  }, [])
  const handleSave = ()=>{
...
return (
  <div>
    <div className={style.mainContainer}>
      ...
      dangerouslySetInnerHTML={{__html:  responseData || results ? results[0]?.result7 : ''}}
    />
              {/* <p>{state?.response || ''}</p> */}
            </div>
