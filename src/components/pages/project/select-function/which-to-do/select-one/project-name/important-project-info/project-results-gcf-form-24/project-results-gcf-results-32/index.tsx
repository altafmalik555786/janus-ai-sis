...
const [responseData] = useState(generateResult || state?.response);
  ...
  useEffect(() => {
    if(localStorage.getItem('allResults') === null){
      localStorage.setItem('allResults', JSON.stringify([{result4: state?.response || ""}]))
    }
  }, [])
...
return (
    <div className={style.mainContainer}>
      ...
      dangerouslySetInnerHTML={{__html:  responseData || results ? results[0]?.result4 : ''}}
    />
              {/* <p>{state?.response || ''}</p> */}
            </div>
            ...
            {/* <div className={style.footerButtonsDiv}>
              ...
              <div className={style.btnDiv}>
                <div className={style.twoBtnDiv}>
                  <button className={style.goBtn} onClick={() => navigate(constRoute?.projectResultsGcfForm24)} >
